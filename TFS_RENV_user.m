...
work.test_stimuli{set.stim_indx} = 'W2ADA7M.wav';
...
elseif chosenNoise == 3
    % Interrupted noise mimicking VC-2 and VC-4
    % Create 10 Hz square-wave window that alternates between 0 and 1, duty cycle of 75%.
...
elseif chosenNoise == 0
    % Interrupted noise mimicking VC-16.
    % Create 2 Hz square-wave window that alternates between 0 and 1, duty cycle of 90%.
    win = 0.5 + 0.5*square(2*pi*2*[0:2*size(n,1)-1]/def.samplerate, 90);
    win = win(max(1,round(size(n,1)*rand(1)))+[0:size(n,1)-1]);
    % Level in gaps is always 30 dB.  Determine level at peaks
    % to yield desired overall level
    peak_level = 10*log10(2*(10^(set.noiseLevel/10)) - 10^(30/10));
