function vad_flag = vadG729(speech, VAD_cst_param_modified))
...
if isempty(biquad)
    ...
    VAD_var_param.window_buffer = single(zeros(VAD_cst_param_modified.L_WINDOW - VAD_cst_param_modified.L_FRAME, 1));
    VAD_var_param.frm_count = single(0);
    VAD_var_param.MeanLSF = single(zeros(VAD_cst_param_modified.M, 1));
    ...
    VAD_var_param.Min_buffer = single(Inf * ones(1, VAD_cst_param_modified.No));    
end
...
% Windowing of signal
speech_win = VAD_cst_param_modified.hamwindow .* speech_buf;

% Autocorrelation
r = autocorr(speech_win) .* VAD_cst_param_modified.lagwindow;

% LSF
a = levSolver1(r(1:VAD_cst_param_modified.M+1));
LSF = lpc2lsf(a) / (2 * pi);
...
% Full-band energy
Ef = 10 * log10(r(1) / VAD_cst_param_modified.L_WINDOW);

% Low-band energy
El = (r(1) * VAD_cst_param_modified.lbf_corr(1) + 2 * sum(r(2:end) .* VAD_cst_param_modified.lbf_corr(2:end))).';
El = 10 * log10(El / VAD_cst_param_modified.L_WINDOW);

% Spectral Distortion
SD = sum((LSF-VAD_var_param.MeanLSF).^2);

% Zero-crossing rate
idx = VAD_cst_param_modified.L_WINDOW - VAD_cst_param_modified.L_NEXT - VAD_cst_param_modified.L_FRAME + 1;
ZC = double(zerodet(speech_buf(idx:idx+VAD_cst_param_modified.L_FRAME)))/VAD_cst_param_modified.L_FRAME;
...
if (VAD_var_param.frm_count < VAD_cst_param_modified.Ni)
    ...
    % Step 4
    if ((Ef < VAD_var_param.MeanSE + 3) && (VAD_var_param.frm_count > VAD_cst_param_modified.No) ...
        && (v_flag == 0) && (rc(2) < 0.6))
        marker = NOISE;
    end
    ...
    if ((Ef < VAD_var_param.MeanSE + 3) && (rc(2) < 0.75) && (SD < 0.002532959))
        ...
        if (VAD_var_param.count_update < VAD_cst_param_modified.INIT_COUNT)
            COEF = single(0.75);
            COEFZC = single(0.8);
            COEFSD = single(0.6);
        elseif (VAD_var_param.count_update < VAD_cst_param_modified.INIT_COUNT + 10)
            COEF = single(0.95);
            COEFZC = single(0.92);
            COEFSD = single(0.65);
        elseif (VAD_var_param.count_update < VAD_cst_param_modified.INIT_COUNT + 20)
            COEF = single(0.97);
            COEFZC = single(0.94);
            COEFSD = single(0.70);
        elseif (VAD_var_param.count_update < VAD_cst_param_modified.INIT_COUNT + 30)
            COEF = single(0.99);
            COEFZC = single(0.96);
            COEFSD = single(0.75);
        elseif (VAD_var_param.count_update < VAD_cst_param_modified.INIT_COUNT + 40)
            COEF = single(0.995);
            COEFZC = single(0.99);
            COEFSD = single(0.75);
        else
    ...
    if ((VAD_var_param.frm_count > VAD_cst_param_modified.No) && ((VAD_var_param.MeanSE < Min) &&  (SD < 0.002532959)) || (VAD_var_param.MeanSE > Min + 10 ))
...
idx = VAD_cst_param_modified.L_FRAME + 1;
